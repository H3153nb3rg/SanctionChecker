 ...
 */
package at.jps.sanction.core.banking.iban;
...
public final class Iban {
     ...
     *             if a valid IBAN could not be calculated using the given IsoCountry and BBAN
     */
    public Iban(final IsoCountry country, final String bban) {
        if (country == null) {
        ...
        final String normalizedBban = normalize(bban);
        final String normalizedIban = country.getCode() + "00" + normalizedBban;

        final BbanStructure bbanStructure = BbanStructure.forCountry(country);
        ...
        final String checkDigits = IbanCheckDigit.INSTANCE.calculate(normalizedIban);

        iban = country.getCode() + checkDigits + normalizedBban;
    }
     ...
     * @throws IbanFormatException
     *             if the given string is not a valid IBAN
     */
    ...
    private static String normalize(final String iban) {
    ...
    private static IsoCountry findCountryFor(final String s) {
    ...
    public String toPrintableString() {
        final StringBuilder printableIban = new StringBuilder(iban);
