...
public enum IbanCheckDigit {
     ...
     *             if the given IBAN is null or if its size is not at least four characters.
     */
     ...
     */
    public String calculate(final String iban) {
        final int modulusResult = modulus(iban);
    ...
    private int modulus(final String iban) {
        if (iban == null) {
        ...
        final String reformattedIban = iban.substring(BBAN_INDEX) + iban.substring(0, BBAN_INDEX);
        long total = 0;
        for (int i = 0; i < reformattedIban.length(); i++) {
            final int charValue = Character.getNumericValue(reformattedIban.charAt(i));
